...
public class GetProductsByCategoryQueryHandler(IAppDbContext context, IInventoryQueries inventoryQueries) : IRequestHandler<GetProductsByCategoryQuery, Envelope<Paginated<ProductDto>>>
{
    public async Task<Envelope<Paginated<ProductDto>>> Handle(GetProductsByCategoryQuery request, CancellationToken cancellationToken)
    {
        var queryable = context.Products.AsNoTracking()
            .Where(c => 
                ...
                (
                    string.IsNullOrWhiteSpace(request.Request.QueryPattern) || 
                    c.Category == null || 
                    c.Category.Slug == request.Request.QueryPattern) 
                )
            .AsQueryable();
