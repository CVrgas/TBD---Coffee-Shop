    ...
    IUnitOfWork uOw) : IPaymentIntentService
{
    ...
    public async Task<Envelope> ConfirmPaymentIntentAsync(string intentId, string orderNumer, CancellationToken ct = default)
    {
        ...
        var record = await payRecordRepository.GetAsync(
            r => r.IntentId == intentId,
            includes: q => q.Include(r => r.Order).ThenInclude(o => o.PaymentRecords),
            asNoTracking: false,
            ct: ct);
        
        if(record is null) return Envelope.NotFound("intent not found.");
        if(!(record.Order.OrderNumber == orderNumer && record.IntentId == intentId)) return Envelope.BadRequest("invalid order number.");
        ...
        await uOw.SaveChangesAsync(ct);
        return confirmation.IsSuccess ? Envelope.Ok() : Envelope.BadRequest(confirmation.ErrorMessage ?? "unknown error");
    }
}